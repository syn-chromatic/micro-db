...
use raw_allocator::buffer::Buffer;
use raw_allocator::buffer::ValidatedOffset;
use raw_allocator::buffer::HEADER_SIZE;

use raw_allocator::entry::Entry;
use raw_allocator::entry::State;
...
impl<const N: usize> Allocator<N> {
    ...
    //     let buffer: &Buffer<N> = &self.raw.lock().buffer;
    //     for offset in buffer.entries() {
    //         let entry: Entry = buffer[offset];
    //         match entry.state() {
    //             State::Free => {}

    //             State::Used => {
    //                 let offset_size: usize = offset.0;
    //                 size = offset_size + entry.size() + HEADER_SIZE;
    //             }
    //         }
    //     }
    //     size
    ...
    pub fn get_used_memory(&self) -> usize {
        ...
        let buffer: &Buffer<N> = &self.raw.lock().buffer;
        let offset: Option<ValidatedOffset> = buffer.entries().last();
        if let Some(offset) = offset {
            let entry: Entry = buffer[offset];
            let state: State = entry.state();
            let offset_size: usize = offset.0;
            ...
            match state {
                ...
                State::Used => {
                    size += offset_size + buffer[offset].size();
                }
            }
        }
    ...
    pub fn get_free_memory(&self) -> usize {
        let total_memory: usize = self.get_total_memory();
        let used_memory: usize = self.get_used_memory();
        total_memory - used_memory
    }

    pub fn get_total_memory(&self) -> usize {
        self.raw.lock().buffer.0.len()
    }
